...
import 'package:app_admin/utils/user_info_dialog.dart';
...
final dashboardTopUsersProvider = FutureProvider<List<UserModel>>((ref) async {
  final List<UserModel> users = await FirebaseService().getTopUsers(5);
  return users;
});

class DashboardTopUsers extends ConsumerWidget {
  const DashboardTopUsers({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final users = ref.watch(dashboardTopUsersProvider);
    return Container(
